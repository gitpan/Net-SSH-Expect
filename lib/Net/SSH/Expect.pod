...
=head4 boolean B<run_ssh()> - forks the ssh client process
...
=head4 string B<login($test_success)>  - authenticates on the ssh server. 
...
=head4 string B<exec($cmd [,$timeout])> - executes a command in the remote machine returning its output

TODO: fill the documentation

=head4 boolean B<waitfor($pattern [,$timeout])>
...
=head4 string B<before()> - returns the "before match" data of the last waitfor() call, or empty string.

TODO: fill the documentation

=head4 string B<match()> - returns the "match" data of the last waitfor() call, or empty string.

TODO: fill the documentation

=head4 string B<after()> - returns the "after match" data of the last waitfor() call, or empty string.

TODO: fill the documentation

=head4 void B<close()> - terminates the ssh connection

TODO: fill the documentation

=head4 void B<send($string)> - sends $string to the SSH server, returns nothing

TODO: fill the documentation

=head4 string B<peek([$timeout])> - returns what is in the input stream without removing anything
...
=head4 string B<eat($string)> - removes all the head of the input stream until $string inclusive.
...
=head4 string B<read_all([$timeout])> - reads and removes all the output from the input stream.

The reading/removing process will be interrupted after $timeout seconds of inactivity
on the input stream.
TODO: fill the documentation

=head4 string B<read_line([$timeout])> - reads the next line from the input stream and returns it. 
...
=head4 void B<restart_timeout_upon_receive( 0 | 1 )> - changes the timeout counter behaviour
...
=head4 Expect B<get_expect()> - returns the internal Expect object
