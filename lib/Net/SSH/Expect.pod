...
=item String B<login($test_success)>  - authenticates on the ssh server. 
...
=item string B<before()> - returns the "before match" data of the last waitfor() call, or empty string.

TODO: fill the documentation

=item string B<match()> - returns the "match" data of the last waitfor() call, or empty string.

TODO: fill the documentation

=item string B<after()> - returns the "after match" data of the last waitfor() call, or empty string.
...
=item B<read_all([$timeout])> - reads and removes all the output from the input stream.

The reading/removing process will be interrupted after $timeout seconds of inactivity
on the input stream.
TODO: fill the documentation

=item B<has_line([$timeout])> - tells if there is one more line on the input stream.

Does not remove the line read.
TODO: fill the documentation

=item B<read_line([$timeout])> - reads the next line from the input stream and returns it. 
